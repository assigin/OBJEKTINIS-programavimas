...
void skirstymas(list<studentas>& mas, list<studentas>& nezino, char& b)
{
    ...
    {
        remove_copy_if(mas.begin(), mas.end(), back_inserter(nezino), patikra_med2);
        mas.erase(remove_if(mas.begin(), mas.end(), patikra_med1), mas.end());
    }
    else//skirstymas pagal vidurki
    {
        remove_copy_if(mas.begin(), mas.end(), back_inserter(nezino), patikra_vid2);
        mas.erase(remove_if(mas.begin(), mas.end(), patikra_vid1), mas.end());
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
